package com.bytecore.vitalcare.platform.shared.interfaces.rest.transform;

import com.bytecore.vitalcare.platform.shared.application.result.ApplicationError;
import com.bytecore.vitalcare.platform.shared.interfaces.rest.resources.ErrorResource;
...
public final class ErrorResponseAssembler {
    ...
    private static String toEntityNameFromErrorCode(String errorCode) {
        if (errorCode.endsWith("_NOT_FOUND")) return errorCode.replace("_NOT_FOUND", "").toLowerCase(Locale.ROOT);
        if (errorCode.endsWith("_CONFLICT")) return errorCode.replace("_CONFLICT", "").toLowerCase(Locale.ROOT);
        return "resource";
    }
